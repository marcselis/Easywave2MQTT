...
namespace Easywave2Mqtt.Mqtt
{
    ...
    {
        private static readonly IMqttClientOptions Options = new MqttClientOptionsBuilder().WithKeepAlivePeriod(TimeSpan.FromSeconds(60)).WithClientId("BasicTestClient").WithTcpServer("192.168.0.12", 1883).WithCredentials("mqtt", "mqtt").WithCommunicationTimeout(TimeSpan.FromSeconds(30)).Build();
                ...
                using(_bus.Subscribe<EasywaveSwitchTurnedOff>(SendSwitchTurnedOff))
                {
                    var result = await _client.SubscribeAsync(
                        new MqttTopicFilterBuilder()
                            .WithAtLeastOnceQoS()
                            .WithTopic("easywave2mqtt/#")
                            .Build(),
                        new MqttTopicFilterBuilder()
                            .WithAtLeastOnceQoS()
                            .WithTopic("mqtt2easywave/#")
                            .Build()
                ...
                case "easywave2mqtt":
                    switch(action)
                    {
                        case "state":
                            break;
                        default:
                            var actionParts = body.Split('_');
                            var buttonCode = actionParts[1][0];
                            _logger.LogInformation("Received {Action} on {Address}:{ButtonCode}", actionParts[2], address, buttonCode);
                            await _bus.PublishAsync(new MqttMessage(address, buttonCode, actionParts[2]));
                            break;
                    }
                    break;
                case "mqtt2easywave":
        ...
        internal async Task Send(string topic, string payload, bool retain=false)
        {
            ...
            MqttClientPublishResult? publishResult = await _client.PublishAsync(new MqttApplicationMessageBuilder().WithTopic(topic).WithPayload(payload).WithQualityOfServiceLevel(MQTTnet.Protocol.MqttQualityOfServiceLevel.AtLeastOnce).WithRetainFlag(retain).Build()).ConfigureAwait(false);
