...
namespace Easywave2Mqtt.Easywave
{
  ...
  public sealed partial class EldatRx09Transceiver : BackgroundService
  {
    ...
    private Task SendEasywaveCommand(SendEasywaveCommand message)
    {
      ...
      _port.WriteLine($"TXP,{address:x2},{message.KeyCode}");
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
      ...
      try
      {
        //await here to be able to continue calling method
    ...
    public partial void LogSendLine(string line);

    [LoggerMessage(EventId = 3, Level = LogLevel.Debug, Message = "Received {Line}")]
    public partial void LogReceivedLine(string line);

    [LoggerMessage(EventId = 4, Level = LogLevel.Information, Message = "Eldat Transceiver detected with identifier {VendorId}:{DeviceId}")]
    public partial void LogEldatDetected(string vendorId, string deviceId);

    [LoggerMessage(EventId = 5, Level = LogLevel.Debug, Message = "Eldat Transceiver confirmed last command")]
    public partial void LogConfirmation();

    [LoggerMessage(EventId = 6, Level = LogLevel.Debug, Message = "Opening serial port {Port}")]
    public partial void LogOpenPort(string port);

    [LoggerMessage(EventId = 97, Level = LogLevel.Trace, Message = "-->{Method} start")]
    public partial void LogMethodStart([CallerMemberName] string method = "");

    [LoggerMessage(EventId = 98, Level = LogLevel.Trace, Message = "-->{Method}({Obj}) start")]
    public partial void LogMethodStart1(string obj, [CallerMemberName] string method = "");
    ...
    public partial void LogMethodEnd1(string result, [CallerMemberName] string method = "");
